...
export type Category = {
  ...
  fetchLimit: number;
  /**
   * Which OSM element types to search. `nwr` covers nodes, ways and relations
   * and is needed for anything mapped as an area — a museum building, a park
   * boundary. For categories that are almost always a single point, `node`
   * cuts the index work to a third, which is the difference between Tokyo
   * answering and timing out.
   */
  elements: "node" | "nwr";
